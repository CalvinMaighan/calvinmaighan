...
## calvinmaighan funnel

Statuses: `Draft` → `Brief` → `Ready` → `To Do` → `In Progress` → `Done`.

- New tasks default to **Draft**. Brief artifact under `_bmad-output/` is **required** before Ready.
- Promote to Ready / To Do is **human-only** (`BACKLOG_ALLOW_PROMOTE=1`). Never run `backlog:ready` or `clickup:todo` as an agent; never set that env var.
- Skills: `backlog-funnel`, `backlog-ready`, `clickup-todo`.
- Never kick agents from Ready/To Do. Agent runs only when ClickUp status is dragged to **Work**.
- Do not mark **Done** for the user; they verify ACs.
